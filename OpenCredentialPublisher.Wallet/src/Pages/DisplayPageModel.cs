...
namespace OpenCredentialPublisher.ClrWallet.Pages
{
    public class DisplayPageModel : PageModel
    {
        ...
        public async Task<IActionResult> CheckVerifiableCredential(CredentialPackageModel package, string id)
        {

            var revocationResult = await _credentialPackageService.CheckRevocationAsync(package);
            if (revocationResult.revoked)
            {
                return new OkObjectResult(new Verification(id, message:
        ...
        protected string SanitizePath(string url)
        {
        ...
        //                var publicKey = sa.Achievement.Issuer.PublicKey;

        //                if (sa.Id == assertionId)
        //                {
